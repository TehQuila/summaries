...
\begin{definition}[Convergent Sequence]\label{def:conv_seq}
   ...
   \[\forall \varepsilon > 0~\exists n_\varepsilon \in \mathbb{N}: (\forall n \geq n_\varepsilon: \abs{x_n - x} < \varepsilon)\]
\end{definition}
\begin{remark}[Intuition]
...
\begin{definition}[Bounded Sequence]
   ...
   \[\text{bounded} :\iff \exists b \in \mathbb{R}_{>0}: (\forall n \in \mathbb{N}: \abs{x_n} \leq b)\]
\end{definition}
\begin{remark}
...
\begin{proposition}[Important Sequences]
   ...
   \begin{enumerate}[label=\roman*, align=Center]
      ...
      \item \[\lim_{n \to \infty}(x^n) = \begin{cases}
         ...
         \end{cases}\]
      \item Let \(\abs{x} < 1\) and \(r \in \mathbb{N}\)
...
\begin{proof}[Proof (i)]
   ...
   \textbf{Case 2.1:} \(\abs{x} < 1\) implies that \(x = \frac{a}{b}\) with \(a < b\), so we regard \(\abs{x_n} = \abs{x^n} = \abs{x}^n\)
   \[\frac{\abs{x_{n+1}}}{\abs{x_n}} = \frac{\abs{\frac{a}{b}}^{n+1}}{\abs{\frac{a}{b}}^n} = \frac{\abs{a}^{n+1}\cdot\abs{b}^n}{\abs{a}^n\cdot\abs{b}^{n+1}} = \abs{\frac{a}{b}} < 1\]
   hence \((\abs{x_n})\) is decreasing and since \(\forall n \in \mathbb{N}: 0 \leq \abs{x_n}\) also bounded below.
   So \(\abs{x_n} \xrightarrow{n \to \infty} 0\), according to \cref{thm:incr_bound_conv} and thus \(\lim_{n \to \infty} x_n = 0\).

   \textbf{Case 2.2:} \(\abs{x} = 1\) but \(x \neq 1\).
   Suppose \((x_n)\) converges, then \(\lim_{n \to \infty} x_n = \tilde{x} = 0\) by assumption.
   So according to \cref{lem:zero_seq} (i) \(\lim_{n \to \infty} \abs{x_n} = 0\).
   But this is a contradiction to \(\abs{x} = 1\), hence \((x_n)\) diverges.

   \textbf{Cases 2.3:} \(\abs{x} > 1 \implies \left(\frac{1}{\abs{x}}\right)^n \xrightarrow{n \to \infty} 0\)
   This means that
   \[\forall \varepsilon > 0~\exists n_\varepsilon \in \mathbb{N}: \left(\forall n > n_\varepsilon: \frac{1}{\abs{x^n}} < \varepsilon\right)\]
...
\begin{proof}[Proof (ii)]
   ...
   We regard \(x_n := n^r \cdot \abs{x}^n\), then \(x_n > 0\) and
   \[\frac{x_{n+1}}{x_n} = \frac{(n+1)^r \cdot \abs{x}^{n+1}}{n^r \cdot \abs{x}^n} = \abs{x} \left(\frac{n+1}{n}\right)^r = \abs{x} \left(1 + \frac{1}{n}\right)^r \xrightarrow{n \to \infty} \abs{x}\]
   since \(\left(1 + \frac{1}{n}\right)^r \xrightarrow{n \to \infty} 1\).

   Now let \(\alpha \in \big(\abs{x}; 1\big)\), then
   \[\exists n_0 \in \mathbb{N}~\forall n > n_0: \left(\frac{x_{n+1}}{x_n} - \alpha = \abs{\frac{x_{n+1}}{x_n} - \alpha} < \alpha - \abs{x} \implies \frac{x_{n+1}}{x_n} < \alpha  \implies x_{n+1} < \alpha \cdot x_n\right)\]
   So by induction \(\forall n > n_0: x_n \leq x_{n_0} \cdot \alpha^{n - n_0}\), hence
   \[\abs{n^r \cdot x^n} = x_n \leq x_{n_0} \cdot \alpha^{n-n_0} \xrightarrow{n \to \infty} 0\]
...
\begin{proposition}[Zero Sequence Rules]\label{lem:zero_seq}
   ...
   \begin{enumerate}[label=\roman*, align=Center]
      ...
      \item \[\exists n_0 \in \mathbb{N}: (\forall n \geq n_0: \abs{x_n} \leq y_n) \implies \lim_{n \to \infty}(x_n) = 0\]
   \end{enumerate}
\end{proposition}
...
\begin{remark}[Tips]
   ...
   \[\forall n, m > n_\varepsilon: \abs{x_m - x_n} < \varepsilon \iff \forall m > n \geq n_\varepsilon: \abs{x_m - x_n} < \varepsilon\]
   and also
   \[\abs{x_m - x_n} = \abs{(x_m - x) + (x - x_n)} \leq \abs{x_m - x} + \abs{x_n - x} < \varepsilon\]
\end{remark}
...
\begin{definition}[Accumulation Point]
   ...
   \[\forall \varepsilon > 0: \exists~\text{an infinite}~N_{\varepsilon} \subset \mathbb{N}: (\forall n \in N_\varepsilon: \abs{x_n - x} < \varepsilon)\]
\end{definition}
\begin{example}
...
\begin{theorem}
   ...
   \[x~\text{an accumulation point of}~x_n \iff \forall \varepsilon > 0: (\forall n \in \mathbb{N}~\exists m \geq n: \abs{x_m - x} < \varepsilon)\]
\end{theorem}
...
Let \(x_n \in \mathbb{R}^\mathbb{N}\) be bounded, i.e. \(\forall n \in \mathbb{N}: \abs{x_n} \leq b\).
We define
...
\begin{theorem}[Cauchy Criterion for Series]\label{thm:cauchy_crit_ser}
   \[\sum x_n~\text{converges} \iff \forall \varepsilon > 0: \exists n_\varepsilon \in \mathbb{N}: \left(\forall n > m \geq n_\varepsilon: \abs{\sum_{k=m+1}^n x_k} < \varepsilon \right)\]
\end{theorem}
...
\begin{definition}[Absolute Convergent Series]
   A series where \(\sum \abs{x_n}\) converges.
\end{definition}
\begin{remark}[Tips]
...
\begin{proposition}[Root Test]\label{pro:root_test}
   Let \((x_n)_{n \in \mathbb{N}}\) and \(\alpha := \limsup_{n \to \infty}\big(\sqrt[n]{\abs{x_n}}\big)\), then
...
\begin{proposition}[Ratio Test]\label{pro:ratio_test}
   ...
   \[\limsup_{n \to \infty} \left(\abs{\frac{x_{n+1}}{x_n}}\right) < 1 \implies \sum x_n~\text{absolute convergent}\]
   \[\liminf_{n \to \infty} \left(\abs{\frac{x_{n+1}}{x_n}}\right) > 1 \implies \sum x_n~\text{divergent}\]
...
\begin{proposition}[Important Series]
   ...
   \begin{enumerate}[label=\roman*, align=Center]
      ...
      \item Geometric Series
         ...
         \[\abs{x} < 1 \implies \sum_{n = 0}^\infty x^n = \frac{1}{1 - x}\]
         \[\abs{x} \geq 1 \implies \sum_{n = 0}^\infty x^n = \infty\]
...
\begin{proof}[Proof (i)]
   ...
   \begin{equation*}
      \begin{split}
         \forall m, n > n_\varepsilon: \abs{s_m - s_n} < \varepsilon \iff & \forall m > n \geq n_\varepsilon: \abs{s_m - s_n} < \varepsilon \iff \\
                                                                      & \forall m > n \geq n_\varepsilon: \abs{\sum_{k=1}^m \frac{1}{k} - \sum_{k=1}^n \frac{1}{k}} < \varepsilon \iff \\
                                                                      & \forall m > n \geq n_\varepsilon: \abs{\sum_{k=n+1}^m \frac{1}{k}} < \varepsilon
      \end{split}
   \end{equation*}
...
\begin{definition}[Radius of Convergence]
   ...
   \[\rho := \frac{1}{\lim\sup_{n \to \infty} \sqrt[n]{\abs{a_n}}}\]
...
\begin{example}
   ...
   The series converges to 0 for all \(\abs{z} < 1\) and diverges for \(\abs{z} \geq 1\).
\end{example}
\begin{example}
   For \(a_n := \frac{1}{n} \implies \sum_{n=0}^\infty \frac{1}{n} z^n\) is \(\rho = 1\).
   The series converges for \(\abs{z} < 1\) and diverges for \(z = 1\).
...
\[z = x + iy = r \cdot \exp(i \theta) \qquad\text{where}~r := \abs{z} = \sqrt{x^2 + y^2}\]
...
\begin{remark}
   ...
   \[\lim_{x \uparrow x_0} f(x) = L \iff \forall \varepsilon~\exists \delta > 0: (x \in D: x_0- \delta < x < x_0 \implies \abs{f(x) - L} < \varepsilon)\]
   \[\lim_{x \downarrow x_0} f(x) = L \iff \forall \varepsilon~\exists \delta > 0: (x \in D: x_0 < x < x_0 + \delta \implies \abs{f(x) - L} < \varepsilon)\]
\end{remark}
\begin{example}
   We regard
...
\begin{definition}[\(\varepsilon-\delta\) Continuity]\label{def:eps_delt_cont}
   ...
   \[\forall \varepsilon > 0~\exists \delta > 0: (\forall x \in D: \abs{x-x_0} < \delta \implies \abs{f(x) - f(x_0)} < \varepsilon)\]
\end{definition}
\begin{remark}[Intuition]
...
\begin{remark}[Terminology]
   ...
   \[\exists \varepsilon > 0~\forall \delta > 0: (\exists x \in D: \abs{x - x_0} < \delta~\text{but}~\abs{f(x) - f(x_0)} \geq \varepsilon)\]
\end{remark}
\begin{example}
   Let \(f: \mathbb{R}\setminus\{1\} \to \mathbb{R}\) be defined as \(f(x) := \frac{x^2+x+1}{x+1}\).
   We prove that \(f\) is continuous in \(x_0 := 1\).
   Let \(\varepsilon > 0\) be arbitrary and fixed, then
...
\begin{proposition}[Continuous Functions]
   ...
   \begin{enumerate}[label=\roman*, align=Center]
      \item \(\norm{\ldots}: X \to \mathbb{R}_{\geq 0}\) is continuous.
...
\end{proposition}
\begin{example}
   Let \(f: [-1;1]\setminus\{0\} \to \mathbb{R}\) be defined as \(f(x) := \frac{\exp(-\frac{1}{\abs{x}})}{x}\)
   Is there a continuous extension of \(f\) on \([-1;1]\)?
   \begin{equation*}
      \begin{split}
         \lim_{x \to \infty} \left(\frac{1}{x}\exp(x)\right) = \infty & \implies \lim_{x \to 0} \left(x \exp\left(\frac{1}{\abs{x}}\right)\right) = \infty \implies \lim_{x \to 0} \left(\frac{1}{x \exp\left(\frac{1}{\abs{x}}\right)}\right) = 0 \implies \\
                                                                      & \implies \lim_{x \to 0} \left(\frac{\exp\left(-\frac{1}{\abs{x}}\right)}{x}\right) = 0
      \end{split}
   \end{equation*}
   hence for \(f(0) := 0\) we have a continuous extension.
\end{example}
...
\begin{definition}[Uniform Continuity]
   ...
   \[\forall \varepsilon > 0~\exists \delta > 0: (\forall x,y \in D: \abs{x - y} < \delta \implies \abs{f(x) - f(y)} < \varepsilon)\]
\end{definition}
\begin{remark}[Intuition]
...
\begin{example}
   ...
   However \(f\) is not uniformly continuous on \([0; \infty)\) because for some fixed \(\abs{x - y}\) can \(\abs{x^2 - y^2} = \abs{x - y}\abs{x + y}\) get arbitrary large if we choose \(x, y\) large enough.
\end{example}

\begin{proposition}[Important Examples]
   The following functions are uniformly continuous
   \begin{enumerate}[label=\roman*, align=Center]
      \item \(\norm{\ldots}: X \to \mathbb{R}_{\geq 0}\) is uniformly continuous.
...
Let \(D \subset \mathbb{K}\), \(x_0 \in D\) be an accumulation point of \(D\) and \((Y, \norm{\ldots})\).
