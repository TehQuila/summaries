...
\begin{theorem}[Monotone \& Bounded \(\implies\) Convergent]\label{thm:incr_bound_conv}
...
\begin{proposition}[Rules for Sequence Limits]
...
%    \[\exists n_1 \in \mathbb{N}: (\forall n,m > n_1: |x_n - x_m| < 1)\]
%    Let \(C := \max\{|x_n| + 1 \mid n > n_1\}\), then \(n \leq n_1: |x_n| \leq C\) and
%    \[n > n_1: |x_n| = |x_n - x_{n_1} + x_{n_1}| \leq |x_n - x_{n_1}| + |x_{n_1}| < 1 + |x_{n_1} \leq C\]
% \end{proof}
...
\end{remark}
% \begin{proof}
%    First we show ,,\(\implies\)``, so suppose \((x_n) \to x\) and let \(\varepsilon > 0\), then
%    \[\exists n_\varepsilon \in \mathbb{N}: \left(\forall n > n_\varepsilon: |x_n - x| < \frac{\varepsilon}{2}\right)\]
%    For \(n, m > n_\varepsilon\) follows
%    \[|x_n - x_m| = |(x_n - x) + (x_m - x)| \leq |x_n - x| + |x_m - x| < \varepsilon\]

%    Now we show ,,\(\impliedby\)``.
%    According to \cref{thm:bolzano} and \cref{thm:cauchy_bound} follows that \((x_n)\) has a accumulation point \(x\).
%    Now Let \(\varepsilon > 0\), then
%    \[\exists n_\varepsilon \in \mathbb{N}: \left(\forall n,m > n_\varepsilon: |x_n - x_m| < \frac{\varepsilon}{2}\right)\]
%    Furthermore
%    \[\exists k_\varepsilon \geq n_\varepsilon: |x - x_{k_\varepsilon}| < \frac{\varepsilon}{2}\]
%    Then follows
%    \[\forall n > n_\varepsilon: |x_n - x| = |x_n - x_{k_\varepsilon} + x_{k_\varepsilon} - x| \leq |x_n - x_{k_\varepsilon}| + |x_{k_\varepsilon} - x| < \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon\]
% \end{proof}
